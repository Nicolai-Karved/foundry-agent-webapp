...
description: Use for authorized authentication and session management assessments, including password policy reviews, session fixation checks, and authentication bypass analysis.
---
...
Identify and assess authentication and session management vulnerabilities in web applications. Broken authentication consistently ranks in the OWASP Top 10 and can lead to account takeover, identity theft, and unauthorized access to sensitive systems. This skill covers testing methodologies for password policies, session handling, multi-factor authentication, and credential management.

## Governance alignment (mandatory)
- Follow the Security Assessment Agent’s scope and rules.
- Use security-assessment-ref/references/auth-session.md as the authoritative governance baseline for auth/session requirements.
- If governance is silent on a control, consult OWASP project best practices:
...
- Authorized test accounts and logging access
...
2. **Authentication Testing Results** - Password policy and lockout findings
...
tokens = []
for i in range(100):
    response = requests.get("https://the-target.url/login")
...
2. Attacker sends link to victim with fixed session:
   https://the-target.url/login?SESSIONID=attacker_session
...
# Token manipulation
https://the-target.url/reset?token=abc123&user=victim
...
https://the-target.url/reset?token=a1b2c3d4e5f6
...
https://the-target.url/reset?token=a1b2c3d4e5f6&email=admin@example.com
